...
# Generate stars as far background
star_distance = 10 * body_radius
density = 100  # Increase for more stars
stars_x = np.random.uniform(-star_distance, star_distance, density)
stars_y = np.random.uniform(-star_distance, star_distance, density)
stars_z = np.random.uniform(-star_distance, star_distance, density)
ax.scatter(stars_x, stars_y, stars_z, color='white', s=0.2, alpha=0.8, zorder=1)

# Generate a sphere for Earth
res = 400 #100 is relatively course, computer starts crying at >300 rip
phi, theta = np.mgrid[0:2*np.pi:res*1j, 0:np.pi:res*0.5j]
xe = body_radius * np.sin(theta) * np.cos(phi)
ye = body_radius * np.sin(theta) * np.sin(phi)
ze = body_radius * np.cos(theta)

# Load the Earth texture
path_to_earth_image = 'C:/Users/alnoo/Desktop/earth.jpg'
earth_texture = plt.imread(path_to_earth_image).astype(np.float32) / 255.0

# Normalize sunlight direction
sun_direction = np.array([-1.0, 0,0])  # Ensure float type for division
sun_direction /= np.linalg.norm(sun_direction)

# Calculate illumination based on sun direction
normals = np.array([xe.flatten(), ye.flatten(), ze.flatten()])
normals /= np.linalg.norm(normals, axis=0)  # Normalize
illumination_raw = np.dot(sun_direction, normals).reshape(xe.shape)

# Apply adjustments for transition sharpness, contrast, and light level
transition_sharpness = 1
contrast = 1.5
overall_light_level = 4

illumination_sharp = np.clip(illumination_raw, 0, 1) ** transition_sharpness
illumination_contrast = np.clip((illumination_sharp - 0.5) * contrast + 0.5, 0, 1)
illumination_final = np.clip(illumination_contrast * overall_light_level, 0, 1)

# Map texture coordinates
tex_x = phi / (2 * np.pi)
tex_y = theta / np.pi
facecolors = earth_texture[np.floor(tex_y * (earth_texture.shape[0] - 1)).astype(int),
                           np.floor(tex_x * (earth_texture.shape[1] - 1)).astype(int)]

# Apply final illumination
illuminated_colors = facecolors * illumination_final[:, :, np.newaxis]

# Plot the Earth
ax.plot_surface(xe, ye, ze, rstride=1, cstride=1, facecolors=illuminated_colors,\
                linewidth=0, antialiased=False,zorder=2,alpha=1)
...
orbits = [[6828,  0,       57.5, 0, 0, 0],\
          [24496, 0.72126, 57.5, 0, 0, 0],
          [42164, 0,          0, 0, 0, 0]]
colours = ['w','w','w']
#colours = ['r','g','b']
n = 1
for orbit in orbits:
    ...
    colours[n-1]+'.',markersize=2,label='Orbit {}'.format(n),zorder=3,alpha=0.5)
